...
COOLDOWN_TTL_SECONDS: dict[str, int] = {
    ...
    # 조합 트리거 (백테스트 채택 11개) — state 기반 발화를 transition 으로
    # 변환하기 위해 충분한 cooldown 부여. 백테스트 main_horizon 대응:
    #   T+5 거래일 가설   → 24h (일별 transition 과 거의 동등)
    #   T+20 거래일 가설  → 48h (QUAL-001 만 해당)
    #   공시 결합        → 24h (DART 와 동일 정책)
    "REV-001": 24 * 3600,
    "REV-002": 24 * 3600,
    "REV-003": 24 * 3600,
    "REV-004": 24 * 3600,
