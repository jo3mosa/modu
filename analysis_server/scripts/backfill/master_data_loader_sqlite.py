...
def update_krx_master_db(db_path=_DEFAULT_DB): # analysis/data/ DB 저장 추후 백엔드 DB 서버로 변경
    ...
    # FDR KRX-DESC 는 일부 종목(지주회사·ETF·일부 KOSPI 메이저주 등)에서 Sector 가 NaN 으로
    # 내려와 dropna 시 005930(삼성전자)·000660(SK하이닉스) 같은 시총 상위 종목이 누락된다.
    ...
    # timeout=30: 다른 프로세스(SQLite Viewer 등)가 임시로 락을 잡았을 때 30초까지 대기.
    # 영구 락(뷰어가 계속 열려있는 상태)은 timeout 으로도 못 푸므로 뷰어를 먼저 닫아야 함.
