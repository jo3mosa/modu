...
`financial_statements` raw 데이터를 활용하고, 라이브 단계에선:
  ...
  4. `daily_fundamentals` 에 (stock, today) row REPLACE INTO
...
# 추후 공용 모듈로 추출 가능 — 지금은 직접 import 로 중복 방지.
...
def _load_fs_row(conn: sqlite3.Connection, stock_code: str, fiscal_year: int):
    """financial_statements 단일 행 (annual report). 없으면 None."""
    conn.row_factory = sqlite3.Row
    cur = conn.execute(
        "SELECT * FROM financial_statements "
        "WHERE stock_code=? AND fiscal_year=? AND reprt_code=?",
        (stock_code, fiscal_year, REPRT_CODE_ANNUAL),
    )
    return cur.fetchone()
...
def run_once(
    ...
    stocks = stock_filter or load_active_stocks(db_path)
    if not stocks:
    ...
    def worker(stock_code: str) -> bool:
        limiter.wait()
        return process_stock(stock_code, date_str, fiscal_year, kis, db_path)
...
def main():
    ...
    run_once(kis, args.db, args.date, args.rate, args.workers, stock_filter)
