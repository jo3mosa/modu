...
# date 는 Postgres DATE 타입 → KIS 스냅샷 (str 'YYYY-MM-DD') 와 직접 비교하려면
# text 로 캐스팅해 기존 SQLite 동작과 동일하게 유지.
_RECENT_CANDLES_SQL = text(
    """
...
def load_active_stocks() -> list[str]:
    """stock_master.is_active=TRUE 인 종목 코드 리스트 (정렬)."""
    with get_engine().connect() as conn:
        rows = conn.execute(_ACTIVE_STOCKS_SQL).fetchall()
    return [r[0] for r in rows]


def load_recent_candles(stock_code: str) -> pd.DataFrame:
    """daily_ohlcv 에서 최근 LOOKBACK_DAYS 일치 캔들. 오래된 → 최신 순 정렬."""
    df = pd.read_sql(
        _RECENT_CANDLES_SQL,
        get_engine(),
        params={"stock_code": stock_code, "limit": LOOKBACK_DAYS},
    )
...
def process_stock(stock_code: str, kis: KisApiClient) -> bool:
    """1 종목 → 지표 계산 → Redis 저장. 성공 True, 실패/skip False."""
    try:
        df = load_recent_candles(stock_code)
        if df.empty:
            return False
...
def run_once(
    ...
    def worker(stock_code: str) -> bool:
        limiter.wait()
        return process_stock(stock_code, kis)
...
def run_forever(
    ...
    while True:
        ...
        try:
            run_once(kis, calls_per_sec, workers)
        except Exception:
...
def main():
    ...
    if args.loop:
        run_forever(args.rate, args.workers)
    else:
        kis = KisApiClient()
        run_once(kis, args.rate, args.workers)
