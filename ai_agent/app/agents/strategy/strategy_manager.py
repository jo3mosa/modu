...
def strategy_manager(state: InvestmentAgentState) -> dict[str, Any]:
    ...
    역할:
    - Decision Manager의 ResearchVerdict를 실행 가능한 FinalDecision으로 변환한다.
    ...
    if verdict is None:
        return _hold(
            ...
            asset=None,
            risk_summary=["Decision Manager 결과 누락"],
            history_context_tokens=history_context_tokens,
        )

    if verdict.recommended_side == "hold":
        return _hold(
            reason=verdict.rationale or "Decision Manager가 hold를 권고했습니다.",
    ...
    if final_decision.action == "trade":
        ...
        if missing:
            return _hold(
                reason=(
                    "Strategy Manager가 trade 결정을 생성했지만 "
    ...
    publish_agent_message(state, "STRATEGY", round_count * 2 + 1, final_decision.user_message or final_decision.reason_summary, stock_code=final_decision.asset or None)
