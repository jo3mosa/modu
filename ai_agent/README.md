...
Bull/Bear 토론 → 전략 종합 → 최종 결정 → 리스크 게이트 → 주문 실행까지
한 번의 그래프 호출로 처리한다.
...
이 모듈은 **결정만 만들고 결정만 발행**한다. 실제 KIS API 호출은 백엔드의 책임이다.
`executor` 노드도 현재 MVP에서는 mock 응답만 만든다.
...
└── app/
    ...
    │   ├── order_request/
    │   │   ├── builder.py               # FinalDecision → OrderRequest (수량 계산 포함)
    │   │   └── schemas.py               # OrderRequest
    │   └── executor/
    │       ├── executor.py              # 주문 호출 (MVP: mock 응답)
...
```
        ...
        flow_status == "hold"      flow_status != "hold"
                      ...
                      END        ┌──────────────────┐
                                 ...
                                 └────────┬─────────┘
                                          │
                          risk_cleared == True   risk_cleared == False
                                          │                │
                                          ▼                ▼
                                 ┌──────────────────┐     END
                                 │     executor     │  OrderRequest 생성 + 주문 호출 (MVP: mock)
                                 └────────┬─────────┘
                                          ▼
                                         END
```
...
| `executor` | `final_decision`, `risk_cleared`, `portfolio_snapshot` | `execution_result` | ❌ |

### 두 개의 hard rule (절대 깨지 않는 가정)

1. **`executor`는 `risk_cleared == True` 일 때만 도달 가능.** 우회 엣지 추가 금지.
2. **`risk_gate`, `executor`, `action/*`, `context/*` 는 LLM을 호출하지 않는다.**
...
| 실행 | `execution_result`, `execution_retry_count` | executor |
...
- **`risk_gate` 우회 엣지 추가**, **`executor`/`risk_gate` 에 LLM 도입**, **AES 키 정책 변경** 은
